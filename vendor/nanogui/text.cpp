/*
    src/Text.cpp -- Fancy text box with builtin regular
...
void Text::setTheme(Theme *theme) {
    Widget::setTheme(theme);
    if (mTheme)
        mFontSize = mTheme->mTextBoxFontSize;
}

Vector2i Text::preferredSize(NVGcontext *ctx) const {
...
bool Text::mouseButtonEvent(const Vector2i &p, int button, bool down,
                               int modifiers) {
...
bool Text::mouseMotionEvent(const Vector2i &p, const Vector2i & /* rel */,
...
bool Text::mouseDragEvent(const Vector2i &p, const Vector2i &/* rel */,
                             int /* button */, int /* modifiers */) {
    mMousePos = p;
    mMouseDragPos = p;
...
bool Text::focusEvent(bool focused) {
...
bool Text::keyboardEvent(int key, int /* scancode */, int action, int modifiers) {
    if (mEditable && focused()) {
...
bool Text::keyboardCharacterEvent(unsigned int codepoint) {
    if (mEditable && focused()) {
...
bool Text::checkFormat(const std::string &input, const std::string &format) {
    if (format.empty())
        return true;
    try {
...
void Text::updateCursor(NVGcontext *, float lastx,
...
float Text::cursorIndex2Position(int index, float lastx,
                                    const NVGglyphPosition *glyphs, int size) {
    float pos = 0;
    if (index == size)
...
int Text::position2CursorIndex(float posx, float lastx,
                                  const NVGglyphPosition *glyphs, int size) {
    int mCursorId = 0;
...
Text::SpinArea Text::spinArea(const Vector2i & pos) {
