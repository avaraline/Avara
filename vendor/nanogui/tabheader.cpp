...
void TabHeader::performLayout(NVGcontext* ctx) {
    ...
    Vector2i currentPosition = Vector2i();
...
void TabHeader::draw(NVGcontext* ctx) {
    ...
    Vector2i activePosition = Vector2i();
...
void TabHeader::drawControls(NVGcontext* ctx) {
    ...
    Vector2f leftIconPos = (Vector2f)mPos + Vector2f(xScaleLeft*theme()->mTabControlWidth, yScaleLeft*(float)mSize.y);
    ...
    Vector2f rightIconPos = (Vector2f)mPos + Vector2f((float)mSize.x, (float)mSize.y*yScaleRight) -
