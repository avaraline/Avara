...
Handle CHuffmanHandler::Compress(Handle sourceHandle) {
    int i;
    unsigned char *p;
    int *t;
...
void CHuffmanHandler::DecodeAll(unsigned char *source, unsigned char *dest) {
    long i;
    long bitData;
    long *bitPointer;
    long bitPosition = 0;
    ...
    while (i--) {
...
#ifdef INTEL_ARCH
        {
            ...
            bitData = (((unsigned long)charPointer[0] << 24) | ((unsigned long)charPointer[1] << 16) |
                          ((unsigned long)charPointer[2] << 8) | (unsigned long)charPointer[3])
                      << (bitPosition & 7);
        }
#else
...
Handle CHuffmanHandler::Uncompress(Handle sourceHandle) {
...
long CHuffmanHandler::GetUncompressedLen(Handle sourceHandle) {
...
void CHuffmanHandler::UncompressTo(Handle sourceHandle, Ptr toPtr) {
    HuffHandleHeader *inHeader;
