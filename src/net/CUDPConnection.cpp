...
void CUDPConnection::IUDPConnection(CUDPComm *theOwner) {
    ...
    latencyHistogram = new SlidingHistogram<float>((size_t)(16 * 1000 / (gCurrentGame->fpsScale*CLASSICFRAMETIME)),
    ...
    if (noErr == Dequeue((QElemPtr)thePacket, &queues[kTransmitQ])) {
        ...
        if (uint16_t(maxValid) <= 10) {
            ...
            if (roundTrip < meanRoundTripTime) {
                ...
                varRoundTripTime = (float)std::min(roundTrip*roundTrip, kInitialRoundTripVar);
            }
            #if PACKET_DEBUG || LATENCY_DEBUG
    ...
    if (thePacket == nullptr) {
        ...
        if (thePacket->serialNumber ==
            ...
            if (pack) { //	The queue already contains this packet.
                ...
                int PACKET_CLEANUP = static_cast<int>(std::floor(4.0/gCurrentGame->fpsScale));   // bigger is NOT better, don't go above 5-ish
