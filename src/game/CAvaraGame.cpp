...
void CAvaraGame::FpsCoefficients(Fixed classicCoeff1, Fixed classicCoeff2,
...
//   x[i+1] = x[i] * a
Fixed CAvaraGame::FpsCoefficient1(Fixed classicCoeff1) {
    if (HandlesFastFPS()) {
...
//   x[i+1] = x[i] + b
Fixed CAvaraGame::FpsCoefficient2(Fixed classicCoeff2) {
    if (HandlesFastFPS()) {
...
//   x[i+1] = x[i] + b
Fixed CAvaraGame::FpsOffset(Fixed classicCoeff2) {
    ...
    return 0.5 * (1/fpsScale - 1) * FpsCoefficient2(classicCoeff2);
}
...
long CAvaraGame::FpsFramesPerClassic(long classicFrames)
{
    if (HandlesFastFPS()) {
...
Fixed CAvaraGame::ClassicCoefficient2(Fixed fpsValue) {
    if (HandlesFastFPS()) {
        // lround rounds both positive and negative values away from zero (adding 0.5 doesn't)
